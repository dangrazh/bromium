...
use crate::{
    error::{ScreenCaptureError, ScreenCaptureResult},
...
fn get_monitor_info_ex_w(h_monitor: HMONITOR) -> ScreenCaptureResult<MONITORINFOEXW> {
...
fn get_dev_mode_w(h_monitor: HMONITOR) -> ScreenCaptureResult<DEVMODEW> {
...
fn get_hi_dpi_scale_factor(h_monitor: HMONITOR) -> ScreenCaptureResult<f32> {
    unsafe {
        ...
        if !current_process_is_dpi_awareness {
            return Err(ScreenCaptureError::new("Process not DPI aware"));
        }
...
fn get_scale_factor(h_monitor: HMONITOR) -> ScreenCaptureResult<f32> {
...
impl ImplMonitor {
    ...
    pub fn from_point(x: i32, y: i32) -> ScreenCaptureResult<ImplMonitor> {
        ...
        if h_monitor.is_invalid() {
            return Err(ScreenCaptureError::new("Not found monitor"));
        }
...
impl ImplMonitor {
    ...
    pub fn video_recorder(&self) -> ScreenCaptureResult<(ImplVideoRecorder, Receiver<Frame>)> {
