...
use crate::{
    ScreenCaptureError, ScreenCaptureResult,
...
pub fn texture_to_frame(
    ...
    unsafe {
        ...
        let copy_texture = {
            ...
            texture.ok_or(ScreenCaptureError::new("CreateTexture2D failed"))?
        };
...
impl ImplVideoRecorder {
    pub fn new(h_monitor: HMONITOR) -> ScreenCaptureResult<(Self, Receiver<Frame>)> {
        unsafe {
    ...
    pub fn on_frame(&self) -> ScreenCaptureResult<()> {
    ...
    pub fn start(&self) -> ScreenCaptureResult<()> {
