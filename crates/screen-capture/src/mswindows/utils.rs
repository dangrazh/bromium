...
use crate::{ScreenCaptureError, error::ScreenCaptureResult};
...
pub(super) fn bgra_to_rgba(mut buffer: Vec<u8>) -> Vec<u8> {
    ...
    for src in buffer.chunks_exact_mut(4) {
        src.swap(0, 2);
        // fix https://github.com/nashaofu/screen_capture/issues/92#issuecomment-1910014951
...
pub(super) fn bgra_to_rgba_image(
    ...
    RgbaImage::from_raw(width, height, bgra_to_rgba(buffer))
        .ok_or_else(|| ScreenCaptureError::new("RgbaImage::from_raw failed"))
}
...
pub(super) fn get_process_is_dpi_awareness(process: HANDLE) -> ScreenCaptureResult<bool> {
    unsafe {
        ...
        let get_process_dpi_awareness_proc_address =
            GetProcAddress(*scope_guard_hmodule, s!("GetProcessDpiAwareness")).ok_or(
                ScreenCaptureError::new("GetProcAddress GetProcessDpiAwareness failed"),
            )?;
...
pub(super) fn load_library(
    lib_filename: PCWSTR,
) -> ScreenCaptureResult<ScopeGuard<HMODULE, impl FnOnce(HMODULE)>> {
    unsafe {
...
pub(super) fn open_process(
    ...
    dw_process_id: u32,
) -> ScreenCaptureResult<ScopeGuard<HANDLE, impl FnOnce(HANDLE)>> {
    unsafe {
...
) -> ScreenCaptureResult<DISPLAYCONFIG_TARGET_DEVICE_NAME> {
    unsafe {
        ...
        Err(ScreenCaptureError::new("Get monitor name failed"))
    }
}

pub fn get_window_info(hwnd: HWND) -> ScreenCaptureResult<WINDOWINFO> {
